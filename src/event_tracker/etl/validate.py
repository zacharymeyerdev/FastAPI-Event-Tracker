...
def validate_rows(rows: list[dict]) -> dict:
    ...
    for i, r in enumerate(rows):
        ...
        if cols != EXPECTED_COLUMNS:
            issues["schema_mismatches"].append({"rows": i, "expected": sorted(EXPECTED_COLUMNS), "actual": sorted(cols)})
    
    for k in EXPECTED_COLUMNS:
        if r.get(k) in (None, ""):
            issues["missing_values"].append({"rows": i, "column": k})

    key = (r.get("ts"), r.get("label"), r.get("value"), r.get("source"))
    seen[key] += 1
    if seen[key] > 1:
        issues["duplicates"].append({"rows": i, "key": key})

    try:
        v = float(r.get("value"))
        if not (VALUE_RANGE[0] <= v <= VALUE_RANGE[1]):
            issues["out_of_range"].append({"rows": i, "value": v, "range": VALUE_RANGE})
    except Exception:
        issues["schema_mismatches"].append({"rows": i, "column": "value", "issue": "not a number"})
